...
class ltls_multilabel_loss_function : public loss_function {
public:
    float loss;
    float gradient;

    ltls_multilabel_loss_function() { }
...
bool multi_compare_model_path_ptr_func(const model_path* l, const model_path* r) { return (*l < *r); }
struct multi_compare_model_path_ptr_functor{ bool operator()(const model_path* l, const model_path* r) const { return (*l < *r); } };

bool multi_compare_path_ptr_func(const path* l, const path* r) { return (*l < *r); }
struct multi_compare_path_ptr_functor{ bool operator()(const path* l, const path* r) const { return (*l < *r); } };

bool multi_compare_vertex_ptr_func(const vertex* l, const vertex* r) { return (*l < *r); }
struct multi_compare_vertex_ptr_functor{ bool operator()(const vertex* l, const vertex* r) const { return (*l < *r); } };
...
struct model {
    ...
    priority_queue<model_path*, vector<model_path*>, multi_compare_model_path_ptr_functor> P_potential_ranking;
    model_path *P;
    model_path **L;
...
struct ltls_multilabel {
    ...
    bool random_policy;
    ltls_multilabel_loss_function *loss_function;
...
void init_models(ltls_multilabel& l){
    D_COUT << "INIT MODELS\n";
...
edge* init_edge(ltls_multilabel& l, vertex *v_out, vertex *v_in){
...
vertex* init_vertex(ltls_multilabel& l){
    vertex *v = new vertex();
    v->v = l.v++;
    v->E[0] = nullptr;
    v->E[1] = nullptr;
...
void init_paths(ltls_multilabel& l){
    D_COUT << "INIT PATHS\n";
...
void init_graph(ltls_multilabel& l){
    D_COUT << "INIT GRAPH\n";
...
void init_grad(ltls_multilabel& l){
    D_COUT << "INIT GRAD\n";

    l.v_source->alpha = l.v_sink->beta = 1;

    for(auto e : l.E) e->v_in->alpha += e->v_out->alpha;
    for(auto e = l.E.rbegin(); e != l.E.rend(); ++e) (*e)->v_out->beta += (*e)->v_in->beta;

    l.v_to_v_alpha = new float*[l.v];
    l.v_to_v_beta = new float*[l.v];
    for(size_t i = 0; i < l.v; ++i){
        l.v_to_v_alpha[i] = new float[l.v];
        l.v_to_v_beta[i] = new float[l.v];
        for(size_t j = 0; j < l.v; ++j) l.v_to_v_alpha[i][j] = l.v_to_v_beta[i][j] = 0;
    }

    for(auto v : l.V){
        l.v_to_v_alpha[v->v][v->v] = l.v_to_v_beta[v->v][v->v] = 1;
        for(auto e : l.E){
            if(e->v_out->v >= v->v) l.v_to_v_alpha[v->v][e->v_in->v] += l.v_to_v_alpha[v->v][e->v_out->v];
        }
        for(auto e = l.E.rbegin(); e != l.E.rend(); ++e){
            if((*e)->v_in->v <= v->v) l.v_to_v_beta[v->v][(*e)->v_out->v] += l.v_to_v_beta[v->v][(*e)->v_in->v];
        }
    }

    for(auto e : l.E){
        e->number_of_labels = e->v_out->alpha * e->v_in->beta;
    }
}
...
inline path* get_path(ltls_multilabel& l, model& m, uint32_t label){
    if(label == 0) label = l.k;
    return m.P[label - 1].p;
}

void update_path(ltls_multilabel& l, model& m, path *p){
    p->score = 0;
    for(auto e : p->E) {
        p->score += m.e_scores[e->e];
        //if(SKIPS_FOR) for(uint32_t i = 0; i < e->skips - 1; ++i) p->score = log(exp(m.e_scores[e->e]) + exp(p->score));
    }

    if(p->score > 1) p->score = 1;
    else if(p->score < -1) p->score = -1;
    p->score = (p->score + 1)/2;

    m.L[p->label - 1]->score = p->score;
}

float get_label_score(ltls_multilabel& l, model& m, uint32_t label){
    if(STATS) ++l.get_path_score_count;
    update_path(l, m, m.P[label - 1].p);
    return m.P[label - 1].score;
}

model_path* path_from_edges(ltls_multilabel& l, model& m, vector<edge*> &E){
...
vector<edge*> get_top_path_from(ltls_multilabel& l, model& m, vertex *v_source){
    ...
    priority_queue<vertex*, vector<vertex*>, multi_compare_vertex_ptr_functor> v_queue;
    ...
    while(!v_queue.empty()){
        ...
        for(uint32_t i = 0; i < MAX_E_OUT; ++i){
            edge *_e = _v->E[i];
            if(_e && !_e->removed){
                float new_length = m.e_scores[_e->e] + _v->score;
...
model_path* get_next_top(ltls_multilabel& l, model& m) {
    ...
    if (m.ranking_size == 0) { // first longest path
        ...
        m.P_ranking.clear();
        m.P_potential_ranking = priority_queue<model_path*, vector<model_path*>, multi_compare_model_path_ptr_functor>();
...
vector<model_path*> top_k_paths(ltls_multilabel& l, model& m, uint32_t top_k){
...
vector<uint32_t> top_k_ensemble(ltls_multilabel& l, uint32_t top_k){
...
void evaluate(ltls_multilabel& l, model& m, base_learner& base, example& ec) {
...
void compute_gradients(ltls_multilabel& l, model& m, COST_SENSITIVE::label &ec_labels){

    for(auto e : l.E){
        e->negatives = e->number_of_labels;
        e->positives = e->gradient = 0;
    }
    ...
    for(auto e : l.E){
        float score = 0;
        for(auto _e : l.E){
            if(_e->e < e->e) score += e->v_in->beta * _e->v_out->alpha * l.v_to_v_beta[e->v_out->v][_e->v_in->v] * m.e_scores[_e->e];
            if(_e->e == e->e) score += e->v_out->alpha * e->v_in->beta * m.e_scores[e->e];
            if(_e->e > e->e) score += e->v_out->alpha * _e->v_in->beta * l.v_to_v_alpha[e->v_in->v][_e->v_out->v] * m.e_scores[_e->e];
        }
        e->gradient = e->positives - e->negatives - score;
    }
...
void update_edges(ltls_multilabel& l, model& m, base_learner& base, example& ec){
...
void learn(ltls_multilabel& l, base_learner& base, example& ec){
...
void predict(ltls_multilabel& l, base_learner& base, example& ec){
...
void finish_example(vw& all, ltls_multilabel& l, example& ec) {
    VW::finish_example(all, &ec);
}

void end_pass(ltls_multilabel& l){
    cout << "end of pass " << l.all->passes_complete << "\n";
}

void finish(ltls_multilabel& l){
    float correct = 0;
    for(size_t i = 0; i < l.p_at_k; ++i) {
    ...
    if(STATS) cout << "get_next_top_count = " << l.get_next_top_count << "\nget_path_score_count = " << l.get_path_score_count << endl;
}

void save_load_graph(ltls_multilabel& l, io_buf& model_file, bool read, bool text){
    D_COUT << "SAVE/LOAD\n";
...
base_learner* ltls_multilabel_setup(vw& all) //learner setup
{
    if (missing_option<size_t, true>(all, "ltls_multilabel", "Log Time Log Space with square loss for multilabel"))
        return nullptr;
    ...
    ltls_multilabel& data = calloc_or_throw<ltls_multilabel>();
    data.k = all.vm["ltls_multilabel"].as<size_t>();
    data.ensemble = 1;
    ...
    // ltls_multilabel parse options
    //------------------------------------------------------------------------------------------------------------------

    learner<ltls_multilabel> *l;
    ...
    *(all.file_options) << " --ensemble " << data.ensemble;
    ...
    // ltls_multilabel init graph and paths
    //------------------------------------------------------------------------------------------------------------------
    ...
    data.loss_function = new ltls_multilabel_loss_function();
